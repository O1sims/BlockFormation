...
\usepackage{tabu,booktabs,rotating}
...
\title{\textbf{Extractive Structures in Networks: \\ Definition, Measurement and Formation\thanks{We acknowledge those who provided helpful and insightful comments at the QUB Workshop on Games and Networks while presenting a previous draft of this paper.}}}

%\date{Preliminary draft \\[2ex] \today}
...
\begin{abstract}
\noindent
It is well-known that critical nodes or ``middlemen'' control information flows in a directed network and take advantage of these positions to extract excess rents from these networks. We extend this concept to \emph{critical node sets} and investigate the potential contestation of these critical node sets by other node sets. This methodology allows us to introduce a game theoretic approach to analyse the formation of these critical node sets and allows the introduction of new \emph{network centrality measures} that are based on potential membership of these critical node sets.
...
\bigskip\noindent
\textbf{Keywords:} Critical nodes; Critical node sets; Network contestability; Centrality measures; Block formation.
...
The Oligarchic faction were less well-organised. The faction acted more disruptively as power was more distributed throughout the network; this is illustrated by the widespread presence of middlemen in this network. Consequently, it was less centralised with multiple competing houses at various local power centres. In particular, the Houses of Guadagni, Bischeri, Peruzzi, and Castellan all occupy critical nodes in Figure 1. Therefore, up to half of the faction have the potential to control certain channels of communication. Furthermore, the houses of Peruzzi and Strozzi have an incentive to form a coalition to control information flows in the Oligarchic subnetwork.

Therefore, there exists more opportunities for power struggles in the Oligarchic faction than the Medician faction. This is indeed representative of the time. It is worth noting that a power struggle did indeed occur in the Medician faction later with respect to the \textit{Pazzi Conspiracy}. And this is conducted through the intervention of the Salviati house, which intermediates the Pazzi and Medici houses in the network.

In this paper we introduce analytical concepts that try to measure these features in any arbitrary directed network. These concepts are supported by two analytical methods, one based on identifying contestation of critical node sets and the other founded in a non-cooperative game theoretic analysis that identifies the stable critical structures that emerge in a network. We report that indeed the resulting measures corroborate the historical analysis of the medieval Florentine marriage network presented above.
...
In this section we introduce the concepts that describe extractive structures in a directed network. In this paper, we view a directed network as representing a system of flows between decision-making nodes. We use notions from standard mathematical graph theory as well as social network analysis throughout our discussion.
...
A critical node is a singleton critical set for some pair of nodes. This implies that such a node exercises complete control over all interaction between these two nodes in the network. A critical node is also referred to as a ``middleman'' in the literature. For a full analysis of the role of middlemen in directed networks we also refer to \citet{SimsGilles2014a}.
...
\draw[very thick, color=blue!40,rounded corners] (9,-1) rectangle (11,6) ;
\draw[color=blue!40] (10,2.5) node {$B_2$} ;

\draw[very thick, color=blue!40,rounded corners] (4,-1) rectangle (6,6) ;
\draw[color=blue!40] (5,2.5) node {$B_1$} ;
...
\\
In the example depicted in Figure 1, critical node 6 is strongly uncontested, since any interaction with node 7 requires intermediation by node 6.
\end{example}
...
Critical sets in a network have at least one intermediation that is uncontested by some alternative set of nodes and, therefore, have full control over this intermediation. This is the subject of the next generalisation of the duality result in \citet{SimsGilles2014a}.
...
\section{Measurement of extractive structures in networks}
...
\subsection{Intermediation measures}
...
\section{The formation of extractive structures in networks}
...
There is direct relationship between such a block formation process and the creation of a multi-sided platform in the sense of \citet{HagiuWright2011}. The resulting block can be interpreted as the provision of a platform provided by its members through which node pairs interact.
...
\subsection{Block formation games}
...
\subsection{Stable formation of block structures}
...
\section{Brokerage centrality analysis}
...
Consider network $D$ on node set $N=\{1,2,3,4,5,6,7\}$ originally shown in Figure 2. Players $2$, $5$, and $6$ are middlemen and there exists $66$ distinct critical sets. However, only $3$ of these critical sets are non-redundant in the sense of Theorem \ref{thm:beta-block}.
...
\end{tikzpicture}
\end{center}
\caption{Middlemen in the network depicted in Figure 2.}
\end{figure}
...
\item[For $\mathbf{c = 1}$:]
There exist four strong Nash equilibria: (1) As above, resulting in the partitioning $\langle \{ 1 \} , \{2,3\} , \{4,5\} , \{ 6 \} , \{ 7 \} \rangle$; (2) An SNE resulting in the partitioning $\langle \{ 1 \} , \{2,3\} , \{4 \}, \{5\} , \{ 6 \} , \{ 7 \} \rangle$; (3) An SNE resulting in the partitioning $\langle \{ 1 \} , \{2\} , \{3\} , \{4,5\} , \{ 6 \} , \{ 7 \} \rangle$; and (4) An SNE that results in the trivial non-cooperation equilibrium in which all nodes act solitary given by $\langle \{ 1 \} , \{ 2\} ,\{ 3\} , \{ 4 \} , \{ 5 \} , \{ 6 \} , \{ 7 \} \rangle$.
\\
...
\section{Node centrality measures based on brokerage}
...
\subsection{Application: The Florentine elite network (redux)}
...
\end{tabular}
\end{center}
\caption{Centrality measures for the Florentine elite network in Figure 1.}
\end{table}
...
\subsection{Application: The 9/11 terrorist networks}
...
\paragraph{Origin of the data used.}
...
The brokerage and criticality results for all 9/11 terrorists are provided in Table~\ref{allterrorists} for reference \textbf{(to be moved to an appendix)}. A shorter table of the most critical terrorists across all time periods can be seen in Table~\ref{criticalterrorists}.
...
\section*{Appendix: Depictions of 9/11 terrorist networks}

\texttt{Explanations of depictions, in particular the colours used.}
